...
const DATA_PATH = path.join(ROOT, "maintenance", "legacy-regions", "canada-regions.json");
...
function validate(data, meshMapper, partition, digitalPartition, qa, municipalOverrides, radioDensity) {
...
function main() {
  ...
  const summary = data && meshMapper && partition && digitalPartition && qa && municipalOverrides && radioDensity
    ? validate(data, meshMapper, partition, digitalPartition, qa, municipalOverrides, radioDensity)
    : null;
  ...
  console.log(`Archived census data validation passed: ${JSON.stringify(summary)}`);
}
