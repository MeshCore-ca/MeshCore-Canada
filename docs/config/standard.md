...
These planning assignments are separate from the ON/QC proposal, not adopted
parts of that rollout. Confirm their use with local operators.
...
### City repeater {#local-repeater}

A repeater serving one IATA region is a city repeater, even on its outer boundary.
Different cities or map outlines with the same IATA code still count as one region.
...
Select edge mode when the repeater regularly talks to repeaters in different
IATA regions. Being near a boundary alone does not make it an edge repeater.
Add neighbouring city scopes only if you want to forward their scoped traffic;
they are optional. For Rigaud, linking
...
This edge repeater carries both city names but only its own province. It drops
...
cities must follow the same unscoped policy, or traffic can go around it.
...
### ON/QC Phase 1 settings {#optional-onqc-standard-settings}
...
ON/QC option also selects 3-byte advert IDs. Leave it unchecked to generate
scope commands without these settings; that is not a complete Phase 1 setup.
...
intended route. `set flood.max.unscoped 3` belongs to **Phase 3**, only if needed
after Phase 2 and announced locally. It is not a default generated by the tool.
...
An edge repeater's `*` must not have `F`.
...
!!! warning "ON/QC: wait for Phase 2"
    ...
    **January 2027**, after the repeaters are ready and the rollout is announced.
    Switching early can stop scoped messages at an unconfigured repeater.

Follow the [rollout order](../proposals/onqc-scopes.md#rollout-order). Bots and
MeshMapper use their city scope at the **end of Phase 1**, once local repeaters
carry it. Personal companions wait for Phase 2.

**When Phase 2 is announced**, with MeshCore app **1.43 or newer**, open
**Settings → Network Settings → Default Region Scope** and select `onqc`.
This lets an initial
flooded direct message, and its path-discovery reply, cross the shared mesh.

| ON/QC traffic | Scope |
| --- | --- |
| Companion default and Public channel | `onqc` |
| Personal test channels | Your city, such as `yow` |
...
Select only regions you need to forward. An oversized profile produces no commands.
...
have changed. Recheck the place, province, and additional regions. Ambiguous
