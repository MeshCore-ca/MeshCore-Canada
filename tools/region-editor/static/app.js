(function () {
  ...
  var basePath = document.querySelector('meta[name="mcc-base-path"]').content;
  ...
  async function fetchJson(path, options) {
    ...
    if (!response.ok) {
      throw new Error(data && data.error ? data.error : "The editor data could not be loaded.");
    }
    return data;
  }
  ...
  async function loadProvince(pruid) {
    ...
    try {
      var results = await Promise.all([
        fetchJson("/api/membership?province=" + encodeURIComponent(pruid), { signal: signal }),
        fetchJson("/api/cells?province=" + encodeURIComponent(pruid), { signal: signal })
      ]);
      var membershipData = results[0];
      var topology = results[1];
      state.province = pruid;
      state.membership = new Map();
      membershipData.rows.forEach(function (row) {
        state.membership.set(String(row.DGUID), row);
      });
      state.baseMembershipSha256 = membershipData.baseMembershipSha256;
  ...
  async function exportProposal() {
    ...
    if (!elements.reason.value.trim()) {
      setValidation("Add a short reason for this boundary change.", "error");
      elements.reason.focus();
      return;
    }
    elements.export.disabled = true;
    setValidation("Validating against the current region membership…", "");
    try {
      var response = await fetch(apiUrl("/api/export"), {
        method: "POST",
        credentials: "same-origin",
        headers: {
        ...
        body: JSON.stringify(localProposal())
      });
      if (response.status === 401) {
        window.location.assign(apiUrl("/login"));
        return;
      ...
      if (!response.ok) {
        ...
        throw new Error(problem.error || "The proposal could not be validated.");
      }
      var blob = await response.blob();
      var disposition = response.headers.get("Content-Disposition") || "";
      var match = disposition.match(/filename="([^"]+)"/);
      var filename = match ? match[1] : "mcc-region-proposal.json";
      ...
      setValidation("Proposal validated and downloaded for review.", "success");
    } catch (error) {
      setValidation(error.message, "error");
    } finally {
      elements.export.disabled = state.proposed.size === 0;
    }
  }
  ...
  async function initialise() {
    ...
    try {
      var initial = await Promise.all([
        ...
        fetchJson("/api/cell-manifest").catch(function () { return null; }),
        fetchJson("/api/partition").catch(function () { return null; })
      ]);
      state.catalog = initial[0];
      state.partition = initial[2];
      populateProvinceOptions(initial[1]);
      if (state.partition) {
        partitionLayer.clearLayers();
        partitionLayer.addData(state.partition);
      }
